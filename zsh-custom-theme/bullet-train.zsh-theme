...
# GIT Commit
if [ ! -n "${BULLETTRAIN_GIT_COMMIT_FG_COLOR+1}" ]; then
  BULLETTRAIN_GIT_COMMIT_FG_COLOR=blue
fi
if [ ! -n "${BULLETTRAIN_GIT_COMMIT_BG_COLOR+1}" ]; then
  BULLETTRAIN_GIT_COMMIT_BG_COLOR=black
fi
if [ ! -n "${BULLETTRAIN_GIT_COMMIT_CMD+1}" ]; then
  BULLETTRAIN_GIT_COMMIT_CMD="\$(git_prompt_short_sha)"
fi
if [ ! -n "${BULLETTRAIN_GIT_COMMIT+1}" ]; then
  BULLETTRAIN_GIT_COMMIT="\\ue821 "
fi
...
prompt_git() {
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    eval git_commit=${BULLETTRAIN_GIT_COMMIT_CMD}
    if [[ $BULLETTRAIN_GIT_COMMIT_SHOW == true ]]; then
      prompt_segment ${BULLETTRAIN_GIT_COMMIT_BG_COLOR} ${BULLETTRAIN_GIT_COMMIT_FG_COLOR}
      echo -n ${BULLETTRAIN_GIT_COMMIT}${git_commit}
    fi
  fi
}
